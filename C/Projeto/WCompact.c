...
static int cmpWords(const void *p1, const void *p2) {
	WordInfo *word1 = (WordInfo *)p1;
	WordInfo *word2 = (WordInfo *)p2;
	int result = word1->count < word2->count ? 1 : word1->count > word2->count? -1 : 0;
...
{
	qsort(words, wordsN, sizeof(WordInfo), cmpWords); 
	String code = {letterFirst(), '\0'};
	for(int i = 0; i < wordsN; i++) {
		strcpy(words[i].code, code);
		codeNext(code);
	}
